...
import { AnimatePresence } from 'framer-motion'; // Assuming AnimatePresence is from framer-motion

// Assuming you have a Notification component like this:
// import Notification from '@/components/Notification'; // Adjust path as needed
// For demonstration, I'll assume Notification is a simple functional component.
// In a real app, you'd import your actual Notification component.
const Notification = ({ type, message, onClose, show }) => {
    if (!show) return null;

    const bgColor = type === 'success' ? 'bg-green-500' : 'bg-red-500';
    return (
        <div className={`fixed bottom-4 right-4 p-4 rounded-lg shadow-lg text-white ${bgColor} z-[100]`}>
...
export default function UserDetails() {
    ...
    // const [password, setPassword] = useState(''); // Not used, consider removing if not needed for admin actions
    ...
    const [actionType, setActionType] = useState(null); // 'credit' or 'debit'
    ...
    const handleAction = async (type) => {
        ...
        setLoading(true); // Start loading for action
        const userRef = doc(db, 'users', id);
        const date = new Date();
        const formattedDate = date.toLocaleString('en-US', { // Changed locale for date formatting if needed
            timeZone: 'America/New_York', // Example time zone for US
            year: 'numeric',
            month: 'short',
            day: 'numeric',
            hour: '2-digit',
    ...
    const handleAdminNoteSave = async () => {
        setLoading(true); // Start loading for note save
        const userRef = doc(db, 'users', id);
        try {
    ...
    const handleEarningStatusChange = async (newStatus) => {
        setLoading(true); // Start loading for earning status change
        const userRef = doc(db, 'users', id);
        try {
            await updateDoc(userRef, { earningStatus: newStatus });
            setEarningStatus(newStatus);
            setNotificationType('success');
        ...
        <DashboardLayout>
            ...
            {showNotification && (
                <Notification
                    ...
                    show={showNotification} // Pass show state directly
                />
            )}

            <div className="p-4 sm:p-6 lg:p-8 space-y-6">
                <button onClick={() => router.back()} className="text-sm text-blue-600 hover:underline">← Back to Users</button>
